...
pub fn execute() -> Result<()> {
    ...
    let local_skills = crate::discovery::scan_local_skills();

    if config.skills.is_empty() && local_skills.is_empty() {
    ...
    if project.is_empty() {
        render_table(&statuses);
        println!("\nNo skills repository configured. Only local skills shown.");
    ...
    let client = match GitLabClient::for_project(base_url, project) {
        ...
        Err(e) => {
            render_table(&statuses);
            eprintln!("\nWarning: Authentication failed: {}. Only local skills shown.", e);
            return Ok(());
        }
    };
...
mod tests {
    ...
    #[test]
    fn test_render_table_with_local_skills() {
        let statuses = vec![
            SkillStatus {
